...
import Shared_vars
openaiclient = OpenAI(base_url = f"{Shared_vars.API_ENDPOINT_URI}v1", api_key=Shared_vars.API_KEY,) 
...
def imagegen(msg):
    ...
    chat_completion = openaiclient.chat.completions.create(model="gpt-4", messages=payload, temperature=0.1, max_tokens=150, stop=["</s>","###", "<|im_end|>", "<|im_start|>"])
    ...
    x = generate(tosend,Shared_vars.config.enabled_features['image_generation']['server_address'] , width=res[0], height=res[1])[0]
    return [x,tosend]